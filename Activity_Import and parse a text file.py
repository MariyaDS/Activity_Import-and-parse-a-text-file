...
# 
# Before using the .split() method, the output is one large continuous string. After using the .split() method, the output is a list of strings, with each element representing a line from the original text.
...
# The entry that was added to the log file appears at the end of the file. Since the file was opened in append mode ("a"), the new entry was added after all existing content, preserving the original order and structure of the log file.
...
# This lab provided hands-on experience with essential file handling operations in Python, including reading from and writing to text files using open() and with statements. Key takeaways include understanding file modes ("r", "w", "a"), using .read() and .write() methods, and managing file content effectively for both appending and reading purposes.
